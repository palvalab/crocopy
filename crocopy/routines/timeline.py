...
from .._base import supports_gpu, get_module, NDArray, HAS_CUPY
...
def _convert_size_to_samples(window_size: int, window_type: str, frequency: float, sampling_rate: int) -> int:
    match window_type:
        case 'samples':
            return window_size
        case 'seconds':
...
def _interfere_function_rank_dtype(data: NDArray[float], function: Callable) -> Tuple[int, np.dtype]:
