...
def compute_fei(data: NDArray[float], window_size: int, overlap: float=0.75, force_gpu: bool=False) -> NDArray[float]:
    ...
    # Replace zero-average windows with NaN to avoid division by zero (e.g. silent channels)
    data_win_averaged_safe = xp.where(data_win_averaged > 0, data_win_averaged, xp.nan)
    profile_windowed /= data_win_averaged_safe[..., None]
