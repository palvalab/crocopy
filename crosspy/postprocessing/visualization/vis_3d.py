...
class BrainSurface:
    ...
    def plot(self, cmap='viridis', camera_position=None, show=True, zoom=1.0, vmin=None, vmax=None, 
        ...
        if type(border_color) is str:
            border_color = mpl.colors.to_rgba(border_color)
        ...
        border_mask = (self.face_labels == -1)

        face_colors = np.full((len(self.face_labels), 4), np.nan) # N_faces x N_channels
        face_colors[border_mask] = border_color
        ...
        no_data_mask = np.isnan(face_colors).any(axis=-1)
        curviture_bin = (self.face_curviture > 0).astype(int)

        no_data_color = np.array([(0.7, 0.7, 0.7, 1.0), (0.35, 0.35, 0.35, 1.0)])
        face_colors[no_data_mask] = no_data_color[curviture_bin][no_data_mask]

        # face_colors[no_data_mask] = curviture_bin[no_data_mask] + len(self.data)
            
        self.brain_mesh.cell_data['colors'] = face_colors

        self.plotter = pv.Plotter(lighting=lightning, off_screen=True)
        self.plotter.add_mesh(self.brain_mesh, scalars='colors', rgba=True, show_scalar_bar=False, silhouette=silhouette)
