...
def _fit_dfa_exponent(window_lengths: NDArray[float], fluct: NDArray[float], weighting: str, N_samp: int, fitting: str='Tukey', min_valid_fraction: float=0.75) -> Tuple[NDArray[float], NDArray[float]]:
    """
...
def _compute_dfa_rms(data, win_lengths, max_nan_frac=0.2, overlap=0.5):
    ...
    """
    xp = get_module(data)
    data = xp.asarray(data, copy=True)
