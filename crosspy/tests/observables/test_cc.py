...
from crosspy.observables.connectivity.amplitude_correlations import compute_cc, compute_occ
...
def test_cc_correctness():
    ...
    res_crosspy = compute_cc(test_data)
    res_numpy = np.corrcoef(np.abs(test_data))

    np.testing.assert_allclose(res_crosspy, res_numpy, atol=1e-4)
